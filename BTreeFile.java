...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		// get internal page
		BTreeInternalPage inPage = (BTreeInternalPage)(this.getPage(tid, dirtypages, pid, Permissions.READ_ONLY));
		Iterator<BTreeEntry> pIt = inPage.iterator();
		BTreeEntry bEntry = null; 
		while(pIt != null && pIt.hasNext()) {
			bEntry = pIt.next();
			if (f == null || f.compare(Op.LESS_THAN_OR_EQ, bEntry.getKey())) {
				return this.findLeafPage(tid, dirtypages, bEntry.getLeftChild(), Permissions.READ_ONLY, f);
			}
		}
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		// get BTreeLeafPage
		BTreeLeafPage rPage = (BTreeLeafPage)(this.getEmptyPage(tid, dirtypages, BTreePageId.LEAF));
		int pNum = page.getNumTuples();
		// move half of page number
		int splitIdx = pNum/2;
        ...
        int count = 0;
		Tuple curTuple = null;
		while (it.hasNext() && count < splitIdx) {
			curTuple = it.next();
		...
		Field upKey = curTuple.getField(this.keyField);
		BTreeInternalPage parPage = (BTreeInternalPage) (this.getParentWithEmptySlots(tid, dirtypages, page.getParentId(), upKey));
		parPage.insertEntry(new BTreeEntry(upKey, page.getId(), rPage.getId()));
		rPage.setParentId(parPage.getId());
		if (page.getRightSiblingId() != null) {
			BTreeLeafPage rSibling = (BTreeLeafPage)(this.getPage(tid, dirtypages, 
					page.getRightSiblingId(), Permissions.READ_WRITE));
			rSibling.setLeftSiblingId(rPage.getId());
			rPage.setRightSiblingId(rSibling.getId());
		}
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		BTreeEntry upEntry = it.next();
		Field upKey = upEntry.getKey();
		page.deleteKeyAndRightChild(upEntry);
		BTreeInternalPage parPage = (BTreeInternalPage)(this.getParentWithEmptySlots(
				tid, dirtypages, page.getParentId(), upKey));
		upEntry.setLeftChild(page.getId());
		upEntry.setRightChild(rPage.getId());
		parPage.insertEntry(upEntry);
		rPage.setParentId(parPage.getId());
		updateParentPointers(tid,dirtypages, rPage);
		if(field.compare(Op.LESS_THAN_OR_EQ, upKey)) return page;
		return rPage;
	}
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		int totalTupleNum = page.getNumTuples() + sibling.getNumTuples();
		int moveNum = totalTupleNum/2 - page.getNumTuples();
		int moveIdx = 0;
		Iterator<Tuple> itor = sibling.reverseIterator();
		if (isRightSibling) itor = sibling.iterator();
		Tuple curT = null;
		while(moveIdx < moveNum) {
			curT = itor.next();
			sibling.deleteTuple(curT);
			page.insertTuple(curT);
			moveIdx++;
		}
		if (isRightSibling) curT = itor.next();
		entry.setKey(curT.getField(keyField));
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// some code goes here
		int totalNum = leftSibling.getNumEntries() + page.getNumEntries();
		int moveNum = totalNum/2 - page.getNumEntries();
		int moveIdx = 0;
		
		BTreePageId lChildId = leftSibling.reverseIterator().next().getRightChild();
		BTreePageId rChildId = page.iterator().next().getLeftChild();
		page.insertEntry(new BTreeEntry(parentEntry.getKey(), lChildId, rChildId));
		
		Iterator<BTreeEntry> itor = leftSibling.reverseIterator();
		BTreeEntry curE = null;
		while(moveIdx < moveNum - 1) {
			curE = itor.next();
			leftSibling.deleteKeyAndRightChild(curE);
			page.insertEntry(curE);
			moveIdx++;
		}
		
		curE = itor.next();
		leftSibling.deleteKeyAndRightChild(curE);
		parentEntry.setKey(curE.getKey());
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int totalNum = rightSibling.getNumEntries() + page.getNumEntries();
		int moveNum = totalNum/2 - page.getNumEntries();
		int moveIdx = 0;
		
		BTreePageId lChildId = page.reverseIterator().next().getRightChild();
		BTreePageId rChildId = rightSibling.iterator().next().getLeftChild();
		page.insertEntry(new BTreeEntry(parentEntry.getKey(),lChildId, rChildId));
		
		Iterator<BTreeEntry> itor = rightSibling.iterator();
		BTreeEntry curE = null;
		while(moveIdx < moveNum - 1) {
			curE = itor.next();
			rightSibling.deleteKeyAndLeftChild(curE);
			page.insertEntry(curE);
			moveIdx++;
		}
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		Iterator<Tuple> itor = rightPage.iterator();
		Tuple curT = null;
		while(itor.hasNext()) {
			curT = itor.next();
			rightPage.deleteTuple(curT);
			leftPage.insertTuple(curT);
		}
		
		BTreePageId rSiblingId = rightPage.getRightSiblingId();
		if(rSiblingId != null) {
			BTreeLeafPage rSibling = (BTreeLeafPage)getPage(tid, dirtypages, rSiblingId, Permissions.READ_ONLY);
			rSibling.setLeftSiblingId(leftPage.getId());
		}
		
		leftPage.setRightSiblingId(rSiblingId);
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		BTreePageId lChildId = leftPage.reverseIterator().next().getRightChild();
		BTreePageId rChildId = rightPage.iterator().next().getLeftChild();
		
		leftPage.insertEntry(new BTreeEntry(parentEntry.getKey(),lChildId, rChildId));
		Iterator<BTreeEntry> itor = rightPage.iterator();
		BTreeEntry curE = null;
		while(itor.hasNext()) {
			curE = itor.next();
